...
<head>
    <meta charset="UTF-8">
    <title>Control Presupuestario PTO. Marketing</title>
    <style>
        body {
            font-size: 12px;
            color: #000;
...
<body>
    <h1>Control PTO. Marketing {{ $year }}</h1>
                        ...
                        @foreach($delegaciones as $delegacion)
                            @php
                                $totalDelegacion = ($totalesPorDelegacion[$delegacion->nombre] ?? 0) + 
                                    array_reduce($ventasPorTrimestre[$trimestre][$mes] ?? [], function ($carry, $producto) use ($delegacion) {
                    ...
                    @foreach($delegaciones as $delegacion)
                        @php
                            $totalTrimestreDelegacion = array_sum(array_column($cajaPorMes, $delegacion->nombre)) + 
                                array_reduce($ventasPorTrimestre[$trimestre] ?? [], function ($carry, $mes) use ($delegacion) {
                                    return $carry + array_reduce($mes, function ($carry, $producto) use ($delegacion) {
                                        return $carry + ($producto['ventasDelegaciones'][$delegacion->nombre]['costeTotal'] ?? 0);
                                    }, 0);
                                }, 0);
